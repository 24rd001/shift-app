...
<table>
<tr>
<th>#</th><th>脆弱性</th><th>深刻度</th><th>内容</th><th>対応</th><th>OWASP</th>
</tr>
<tr>
<td>1</td>
<td><b>権限昇格</b></td>
...
<tr>
<td>3</td>
<td><b>認証情報の<br>クライアント保存</b></td>
<td>🔴 高</td>
<td>ログイン情報（メール・パスワード）をブラウザの localStorage に保存しており、XSS と組み合わさると窃取される恐れがあった</td>
<td>パスワードのクライアント保存を廃止。ログイン情報をブラウザに残さない方式へ変更した</td>
<td>A07</td>
</tr>
<tr>
<td>4</td>
...
<tr>
<td>5</td>
<td><b>セキュリティ<br>ヘッダー未設定</b></td>
<td>🟡 低</td>
<td>Content-Security-Policy（CSP）が未設定で、XSS が起きた際の多層防御が欠けていた</td>
<td>CSP を設定し、スクリプトの実行元を制限。XSS に対する二重の防御を追加した</td>
<td>A05</td>
</tr>
</table>

> このほか、CORS 設定の限定・交代申請の宛先検証・デバッグログの除去・依存パッケージの更新（IDOR 対策を含む）についても、継続的に対応を進めています。
...
- CORS 許可オリジンの限定、交代申請の宛先検証、デバッグログの除去なども順次対応
...
- パスワードのブラウザ保存を廃止し、Content-Security-Policy（CSP）を設定
